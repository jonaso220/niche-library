import type { Perfume, Gender, Concentration } from '@/types/perfume'
...
function mapGender(raw?: string): Gender {
  ...
  if (lower.includes('men') || lower.includes('male') || lower.includes('him')) return 'masculino'
  if (lower.includes('women') || lower.includes('female') || lower.includes('her')) return 'femenino'
  return 'unisex'
}

function mapConcentration(raw?: string): Concentration {
  if (!raw) return 'Other'
